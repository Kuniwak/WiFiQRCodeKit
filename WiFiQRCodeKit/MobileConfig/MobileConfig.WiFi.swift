...
public extension MobileConfig {
    public struct WiFi: Equatable {
        ...
        public let version: PayloadVersion
        ...
        public var serializableRepresentation: PlistSerializable {
            ...
            result[CommonKey.type] = self.type.serializableRepresentation
            result[CommonKey.version] = self.version.serializableRepresentation
            result[CommonKey.identifier] = self.identifier.serializableRepresentation
            result[CommonKey.uuid] = .from(self.uuid.uuidString)
            result[CommonKey.displayName] = self.displayName.serializableRepresentation

            if let description = self.description {
                result[CommonKey.description] = .from(description)
            }

            if let organization = self.organization {
                result[CommonKey.organization] = organization.serializableRepresentation
            }
...
extension MobileConfig.WiFi.BundleIdentifier: Comparable {
    public static func <(
        lhs: MobileConfig.WiFi.BundleIdentifier,
        rhs: MobileConfig.WiFi.BundleIdentifier
    ) -> Bool {
